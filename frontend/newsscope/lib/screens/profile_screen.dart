...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _errorMessage = 'Failed to load profile: $e'; _loading = false; });
    }
  }
    ...
    if (_profile == null || _profile!.isEmpty) {
      return Scaffold(
        ...
        body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text('Start reading articles to build\nyour personal bias breakdown.',
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              _buildUserHeader(),
              const SizedBox(height: 20),
              // Three stat cards: Political Bias | General Bias | Sentiment
  ...
  // ── Three stat cards ───────────────────────────────────────────────────────
  // Card 1: Political Bias  (left/centre/right label + colour)
  // Card 2: General Bias    (Biased / Unbiased derived from leaning distribution)
  // Card 3: Sentiment       (Positive / Neutral / Negative)
  ...
  Widget _buildThreeStatCards() {
    ...
    // General bias — derived from left+right share
    ...
    final generalBiasIcon  =
        extremeRatio > 0.6 ? Icons.warning_amber : Icons.check_circle_outline;
    ...
    return Row(children: [
      Expanded(child: _buildStatCard(
          'Political Bias', biasLabel, biasColor, Icons.balance)),
      const SizedBox(width: 8),
      Expanded(child: _buildStatCard(
          'General Bias', generalBiasLabel, generalBiasColor, generalBiasIcon)),
      const SizedBox(width: 8),
      Expanded(child: _buildStatCard(
          'Sentiment', p.sentimentLabel, sentimentColor, sentimentIcon)),
    ]);
  }
  ...
  List<int> _roundToHundred(double a, double b, double c) {
    ...
    for (int i = 0; i < remainder; i++) { floors[indices[i]]++; }
    return floors;
  }
        ...
        child: Column(children: [
          SizedBox(
            ...
            child: PieChart(PieChartData(
              ...
              sections: [
                if (left > 0) PieChartSectionData(
                    value: leftRaw, title: '$left%',
                    color: Colors.blue[800]!, radius: 55,
                    titleStyle: const TextStyle(fontSize: 13,
                        fontWeight: FontWeight.bold, color: Colors.white)),
                if (center > 0) PieChartSectionData(
                    value: centerRaw, title: '$center%',
                    color: Colors.teal[600]!, radius: 55,
                    titleStyle: const TextStyle(fontSize: 13,
                        fontWeight: FontWeight.bold, color: Colors.white)),
                if (right > 0) PieChartSectionData(
                    value: rightRaw, title: '$right%',
                    color: Colors.red[800]!, radius: 55,
                    titleStyle: const TextStyle(fontSize: 13,
                        fontWeight: FontWeight.bold, color: Colors.white)),
              ],
            )),
  ...
  // ── Source bar chart ───────────────────────────────────────────────────────
  // Sorted descending (rank #1 on left), scrollable, rank badge + full
  // multi-line wrapped source name below each bar.

  Color _sourceBarColor(int index, int total) {
    final palette = [
      Colors.blue[800]!,  Colors.blue[600]!,  Colors.cyan[600]!,
      Colors.teal[500]!,  Colors.teal[700]!,  Colors.indigo[500]!,
      Colors.purple[400]!, Colors.orange[600]!, Colors.orange[400]!,
      Colors.red[400]!,   Colors.red[700]!,   Colors.red[900]!,
    ];
                ...
                child: BarChart(BarChartData(
                  ...
                  titlesData: FlTitlesData(
                    leftTitles: AxisTitles(sideTitles: SideTitles(
                      showTitles: true, reservedSize: 28, interval: interval,
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            return Padding(
                              ...
                              child: Column(mainAxisSize: MainAxisSize.min,
                                ...
                                const SizedBox(height: 3),
                                // Full wrapped source name
                                Text(_wrapSourceName(sources[i].key),
                                  textAlign: TextAlign.center,
                                  style: TextStyle(
                                    fontSize: 9,
  ...
  // ── Detailed breakdown ─────────────────────────────────────────────────────
  // 1. Political leaning card  — 3-column grid with counts, %, stacked bar
  // 2. Sentiment card          — 2-column grid with counts, %, stacked bar
  // 3. Summary strip           — 4 quick stats across a single card
    ...
    return Column(children: [
      ...
      Card(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Icon(Icons.balance, size: 16, color: Colors.blue[700]),
              const SizedBox(width: 8),
            ...
            Row(children: [
              Expanded(child: _buildLeaningTile(
                  label: 'Left Wing', count: p.leftCount, total: total,
                  color: Colors.blue[800]!, icon: Icons.west)),
              const SizedBox(width: 10),
              Expanded(child: _buildLeaningTile(
                  label: 'Centre', count: p.centerCount, total: total,
                  color: Colors.teal[600]!, icon: Icons.remove)),
              const SizedBox(width: 10),
              Expanded(child: _buildLeaningTile(
                  label: 'Right Wing', count: p.rightCount, total: total,
                  color: Colors.red[800]!, icon: Icons.east)),
            ]),
            if (total > 0) ...[
              const SizedBox(height: 14),
              ClipRRect(
                ...
                child: Row(children: [
                  if (p.leftCount > 0)
                    Expanded(flex: p.leftCount,
                        child: Container(height: 8, color: Colors.blue[800])),
                  if (p.centerCount > 0)
                    Expanded(flex: p.centerCount,
                        child: Container(height: 8, color: Colors.teal[600])),
                  if (p.rightCount > 0)
                    Expanded(flex: p.rightCount,
                        child: Container(height: 8, color: Colors.red[800])),
                ]),
              ),
            ],
          ]),
      ...
      Card(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Icon(Icons.sentiment_satisfied, size: 16, color: Colors.green[700]),
              const SizedBox(width: 8),
              Text('Sentiment',
                  style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold,
                      color: Colors.grey[800])),
            ...
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: _buildSentimentTile(
                  label: 'Positive', count: p.positiveCount,
                  total: sentimentTotal, color: Colors.green[700]!,
                  icon: Icons.sentiment_satisfied)),
              const SizedBox(width: 10),
              Expanded(child: _buildSentimentTile(
                  label: 'Negative', count: p.negativeCount,
                  total: sentimentTotal, color: Colors.deepOrange[600]!,
              ...
              ClipRRect(
                ...
                child: Row(children: [
                  if (p.positiveCount > 0)
                    Expanded(flex: p.positiveCount,
                        child: Container(height: 8, color: Colors.green[700])),
                  if (p.negativeCount > 0)
                    Expanded(flex: p.negativeCount,
  ...
  Widget _buildVerticalDivider() {
    return Container(
        width: 1, height: 52, color: Colors.grey[200],
